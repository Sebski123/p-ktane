    ...
    def generatemsg(self):
        ...
        if self.readData:
            if self.message_bytes_ms[0] == 0x00:
                ...
                msgs = bytes_to_msgs(self.current_address, self.message_bytes_ms[1:idx])
                self.putxms([1, msgs])
                self.startmsg = self.prevmsg[0]
                self.endmsg = self.prevmsg[1]
                self.putxms([0, self.prevmsg[2]])
            ...
            if(self.message_bytes_ms[0] != 0x82):
                ...
                if(len(stripped_bytes) > 1 and stripped_bytes[0] == 0xC2):
                    ...
                    msgs = [id_to_module_type[self.current_address] + ":WRITE CONFIG " + config_str, "0x" + str(self.current_address) +":W C " + config_str]
                else:
